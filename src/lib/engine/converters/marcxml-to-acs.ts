...
/**
 * MARCXML → ACS. Parses a MARCXML export into the unified Citation
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert MARCXML to ACS",
        err,
      );
    }
    opts?.onProgress?.(1);
