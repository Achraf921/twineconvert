...
/**
 * ODS → IEEE. Reads the first sheet of a LibreOffice/OpenOffice Calc
 * spreadsheet of references (alias-aware headers), then renders a formatted
 * IEEE reference list with citeproc-js and the official IEEE CSL style.
 */
const odsToIeee: Converter = {
  id: "ods-to-ieee",
  label: "ODS → IEEE",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert ODS to IEEE",
        err,
      );
    }
    opts?.onProgress?.(1);
