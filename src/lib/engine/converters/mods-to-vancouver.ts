...
/**
 * MODS → Vancouver. Parses a MODS export into the unified Citation
 ...
 */
const modsToVancouver: Converter = {
  id: "mods-to-vancouver",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert MODS to Vancouver",
        err,
      );
    }
    opts?.onProgress?.(1);
...
export default modsToVancouver;
