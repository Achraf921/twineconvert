...
/**
 * XLSX → Nature. Reads the first sheet of an Excel spreadsheet of references,
 * then renders a formatted Nature journal style reference list with citeproc-js and the
 * official Nature CSL style.
 */
const xlsxToNature: Converter = {
  id: "xlsx-to-nature",
  label: "XLSX → Nature",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert XLSX to Nature",
        err,
      );
    }
    opts?.onProgress?.(1);
