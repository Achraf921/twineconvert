...
/**
 * XLSX → IEEE. Reads the first sheet of an Excel spreadsheet of
 * references (alias-aware headers), then renders a formatted IEEE
 * reference list with citeproc-js and the official IEEE CSL style.
 */
const xlsxToIeee: Converter = {
  id: "xlsx-to-ieee",
  label: "XLSX → IEEE",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert XLSX to IEEE",
        err,
      );
    }
    opts?.onProgress?.(1);
