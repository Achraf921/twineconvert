...
/**
 * RefWorks → Vancouver. Parses a RefWorks export into the unified Citation
 ...
 */
const refworksToVancouver: Converter = {
  id: "refworks-to-vancouver",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert RefWorks to Vancouver",
        err,
      );
    }
    opts?.onProgress?.(1);
...
export default refworksToVancouver;
