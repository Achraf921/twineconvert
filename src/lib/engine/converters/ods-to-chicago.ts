...
/**
 * ODS → Chicago. Reads the first sheet of a LibreOffice/OpenOffice Calc
 * spreadsheet of references (alias-aware headers), then renders a formatted
 * Chicago reference list with citeproc-js and the official Chicago CSL style.
 */
const odsToChicago: Converter = {
  id: "ods-to-chicago",
  label: "ODS → Chicago",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert ODS to Chicago",
        err,
      );
    }
    opts?.onProgress?.(1);
