...
/**
 * CSV → Harvard in-text citations. Reads a spreadsheet of references
 * (alias-aware headers), then renders the Harvard in-text citation for each
 * row, tab-paired with the source title.
 */
const csvToHarvardIntext: Converter = {
  id: "csv-to-harvard-intext",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(hint ?? (err instanceof Error ? err.message : "Could not generate Harvard in-text citations"), err);
    }
    opts?.onProgress?.(1);
...
export default csvToHarvardIntext;
