...
/**
 * MODS → NBIB. Converts through the unified Citation model so every field
 * carries across the citation family.
 */
const modsToNbib: Converter = {
  id: "mods-to-nbib",
  label: "MODS → NBIB",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert MODS to NBIB",
        err,
      );
    }
    opts?.onProgress?.(1);
