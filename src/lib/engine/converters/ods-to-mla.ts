...
/**
 * ODS → MLA. Reads the first sheet of a LibreOffice/OpenOffice Calc
 * spreadsheet of references (alias-aware headers), then renders a formatted
 * MLA reference list with citeproc-js and the official MLA CSL style.
 */
const odsToMla: Converter = {
  id: "ods-to-mla",
  label: "ODS → MLA",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert ODS to MLA",
        err,
      );
    }
    opts?.onProgress?.(1);
