...
/**
 * ODS → EndNote XML. Reads the first sheet of the ODS spreadsheet of
 ...
 */
const odsToEndnoteXml: Converter = {
  id: "ods-to-endnote-xml",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert ODS to EndNote XML",
        err,
      );
    }
    opts?.onProgress?.(1);
...
export default odsToEndnoteXml;
