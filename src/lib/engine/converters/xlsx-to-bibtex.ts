...
/**
 * XLSX → BibTeX. Reads the first sheet of the XLSX spreadsheet of
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert XLSX to BibTeX",
        err,
      );
    }
    opts?.onProgress?.(1);
