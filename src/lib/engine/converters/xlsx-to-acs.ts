...
/**
 * XLSX → ACS. Reads the first sheet of an Excel spreadsheet of references,
 * then renders a formatted ACS (American Chemical Society) reference list with citeproc-js and the
 * official ACS CSL style.
 */
const xlsxToAcs: Converter = {
  id: "xlsx-to-acs",
  label: "XLSX → ACS",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert XLSX to ACS",
        err,
      );
    }
    opts?.onProgress?.(1);
