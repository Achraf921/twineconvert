...
/**
 * BibTeX → IEEE. Parses BibTeX into the unified Citation model, then
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert BibTeX to IEEE",
        err,
      );
    }
    opts?.onProgress?.(1);
