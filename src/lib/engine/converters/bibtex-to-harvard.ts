...
/**
 * BibTeX → Harvard. Parses BibTeX into the unified Citation model, then
 ...
 */
const bibtexToHarvard: Converter = {
  id: "bibtex-to-harvard",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert BibTeX to Harvard",
        err,
      );
    }
    opts?.onProgress?.(1);
...
export default bibtexToHarvard;
