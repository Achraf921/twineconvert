...
/**
 * Web of Science → NBIB. Parses the Web of Science / ISI tagged export into the
 * unified Citation model, then writes NBIB. The
 * format VOSviewer, bibliometrix, and CiteSpace read; import-only.
 */
const wosToNbib: Converter = {
  id: "wos-to-nbib",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert Web of Science to NBIB",
        err,
      );
    }
    opts?.onProgress?.(1);
