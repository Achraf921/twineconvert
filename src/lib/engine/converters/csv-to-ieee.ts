...
/**
 * CSV → IEEE. Reads a spreadsheet of references (alias-aware headers, so
 * Scopus/Zotero/PubMed CSV exports work), then renders a formatted IEEE
 * reference list with citeproc-js and the official IEEE CSL style.
 */
const csvToIeee: Converter = {
  id: "csv-to-ieee",
  label: "CSV → IEEE",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        hint ?? (err instanceof Error ? err.message : "Could not convert CSV to IEEE"),
        err,
      );
    }
    opts?.onProgress?.(1);
