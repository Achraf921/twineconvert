...
/**
 * XLSX → AMA. Reads the first sheet of an Excel spreadsheet of references
 * (alias-aware headers), then renders a formatted AMA reference list with
 * citeproc-js and the official AMA CSL style.
 */
const xlsxToAma: Converter = {
  id: "xlsx-to-ama",
  label: "XLSX → AMA",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert XLSX to AMA",
        err,
      );
    }
    opts?.onProgress?.(1);
