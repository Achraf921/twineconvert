...
/**
 * XLSX → EndNote XML. Reads the first sheet of the XLSX spreadsheet of
 * references (columns like title, author, year, journal, doi), then
 * writes EndNote XML via the unified Citation model. Column headers are
 * matched case- and alias-insensitively, so PubMed/Zotero/Excel exports
 * work without renaming columns.
 */
const xlsxToEndnoteXml: Converter = {
  id: "xlsx-to-endnote-xml",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert XLSX to EndNote XML",
        err,
      );
    }
    opts?.onProgress?.(1);
...
export default xlsxToEndnoteXml;
