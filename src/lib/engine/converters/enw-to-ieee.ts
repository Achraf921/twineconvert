...
/**
 * EndNote (.enw) → IEEE. Parses an EndNote export into the unified Citation
 * model, then renders a formatted IEEE reference list with citeproc-js and
 * the official IEEE CSL style. Lets EndNote users get a formatted IEEE
 * bibliography without opening EndNote. Plain text.
 */
const enwToIeee: Converter = {
  id: "enw-to-ieee",
  label: "EndNote → IEEE",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert EndNote to IEEE",
        err,
      );
    }
    opts?.onProgress?.(1);
