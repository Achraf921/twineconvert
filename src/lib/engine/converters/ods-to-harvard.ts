...
/**
 * ODS → Harvard. Reads the first sheet of a LibreOffice/OpenOffice Calc
 * spreadsheet of references (alias-aware headers), then renders a formatted
 * Harvard reference list with citeproc-js and the official Harvard CSL style.
 */
const odsToHarvard: Converter = {
  id: "ods-to-harvard",
  label: "ODS → Harvard",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert ODS to Harvard",
        err,
      );
    }
    opts?.onProgress?.(1);
