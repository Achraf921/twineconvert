...
/**
 * RIS → Harvard. Parses RIS into the unified Citation model, then
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert RIS to Harvard",
        err,
      );
    }
    opts?.onProgress?.(1);
