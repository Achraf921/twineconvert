...
const flacToMp3: Converter = {
  id: "flac-to-mp3",
  label: "FLAC → MP3",
  fromMime: ["audio/flac", "audio/x-flac"],
  toMime: "audio/mpeg",
  accept: [".flac"],
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(err instanceof Error && err.message ? `FFmpeg FLAC→MP3 failed: ${err.message}` : "FFmpeg FLAC→MP3 failed", err);
    }
    opts?.onProgress?.(1);
