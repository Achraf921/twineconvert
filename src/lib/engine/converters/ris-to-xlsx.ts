...
const risToXlsx: Converter = {
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      if (citations.length === 0) throw new Error("No references found in RIS file");
      const csv = await citationsToCsv(citations);
      buf = await csvStringToXlsx(csv);
    } catch (err) {
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert RIS to XLSX",
        err,
      );
    }
    opts?.onProgress?.(1);
