...
/**
 * DAE to glTF. COLLADA geometry is flattened through node transforms and merged; materials and animations are not carried over.
 */
const daeToGltf: Converter = {
  id: "dae-to-gltf",
  label: "DAE → glTF",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert DAE to glTF",
        err,
      );
    }
    opts?.onProgress?.(1);
