...
/**
 * RIS to CITATION.cff. Parses RIS and writes a Citation File Format
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert RIS to CFF",
        err,
      );
    }
    opts?.onProgress?.(1);
