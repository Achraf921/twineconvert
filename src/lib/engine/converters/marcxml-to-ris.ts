...
/**
 * MARCXML → RIS. Parses MARC21 slim XML (the format library catalogs export)
 * into the unified Citation model, then writes RIS.
 * Import-only.
 */
const marcxmlToRis: Converter = {
  id: "marcxml-to-ris",
  label: "MARCXML → RIS",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert MARCXML to RIS",
        err,
      );
    }
    opts?.onProgress?.(1);
