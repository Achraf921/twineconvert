...
/**
 * ODS → APA. Reads the first sheet of a LibreOffice/OpenOffice Calc
 * spreadsheet of references (alias-aware headers), then renders a formatted
 * APA reference list with citeproc-js and the official APA CSL style.
 */
const odsToApa: Converter = {
  id: "ods-to-apa",
  label: "ODS → APA",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert ODS to APA",
        err,
      );
    }
    opts?.onProgress?.(1);
