...
/**
 * ODS → ACS. Reads the first sheet of a LibreOffice/OpenOffice Calc
 * spreadsheet of references (alias-aware headers), then renders a formatted
 * ACS reference list with citeproc-js and the official ACS CSL style.
 */
const odsToAcs: Converter = {
  id: "ods-to-acs",
  label: "ODS → ACS",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert ODS to ACS",
        err,
      );
    }
    opts?.onProgress?.(1);
