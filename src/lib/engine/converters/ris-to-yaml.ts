...
/**
 * RIS → CSL-YAML for Pandoc bibliographies (`--bibliography refs.yaml`). Parses RIS into the unified Citation model, then emits the CSL list as YAML.
 */
const risToYaml: Converter = {
  id: "ris-to-yaml",
  label: "RIS → YAML (CSL)",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert RIS to YAML (CSL)",
        err,
      );
    }
    opts?.onProgress?.(1);
