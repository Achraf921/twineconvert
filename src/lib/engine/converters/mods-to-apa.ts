...
/**
 * MODS → APA. Parses a MODS export into the unified Citation
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert MODS to APA",
        err,
      );
    }
    opts?.onProgress?.(1);
