...
/**
 * ODS → Vancouver. Reads the first sheet of a LibreOffice/OpenOffice Calc
 * spreadsheet of references (alias-aware headers), then renders a formatted
 * Vancouver reference list with citeproc-js and the official Vancouver CSL style.
 */
const odsToVancouver: Converter = {
  id: "ods-to-vancouver",
  label: "ODS → Vancouver",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert ODS to Vancouver",
        err,
      );
    }
    opts?.onProgress?.(1);
