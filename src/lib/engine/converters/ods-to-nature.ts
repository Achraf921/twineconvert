...
/**
 * ODS → Nature. Reads the first sheet of a LibreOffice/OpenOffice Calc
 * spreadsheet of references (alias-aware headers), then renders a formatted
 * Nature reference list with citeproc-js and the official Nature CSL style.
 */
const odsToNature: Converter = {
  id: "ods-to-nature",
  label: "ODS → Nature",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert ODS to Nature",
        err,
      );
    }
    opts?.onProgress?.(1);
