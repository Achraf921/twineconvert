...
/**
 * NBIB → Chicago. Parses NBIB into the unified Citation model, then
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert NBIB to Chicago",
        err,
      );
    }
    opts?.onProgress?.(1);
