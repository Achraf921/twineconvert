...
/**
 * Reference list (plain text) → IEEE. Parses a pasted bibliography
 * (numbered IEEE or APA entries) into the unified Citation model, then
 * re-renders it as a clean IEEE reference list with citeproc-js and the
 * official IEEE CSL style. Best-effort: free-text parsing is fuzzy, so
 * confirm the result, but it reformats a rough list into IEEE in one step.
 */
const referencesToIeee: Converter = {
  id: "references-to-ieee",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert references to IEEE",
        err,
      );
    }
    opts?.onProgress?.(1);
...
export default referencesToIeee;
