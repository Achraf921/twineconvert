...
/**
 * CSV → Chicago. Reads a spreadsheet of references (alias-aware headers, so
 ...
 * official Chicago CSL style. Plain text, one reference per paragraph.
 */
const csvToChicago: Converter = {
  id: "csv-to-chicago",
  label: "CSV → Chicago",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        hint ?? (err instanceof Error ? err.message : "Could not convert CSV to Chicago"),
        err,
      );
    }
    opts?.onProgress?.(1);
