...
/**
 * ODS → ASA. Reads the first sheet of a LibreOffice/OpenOffice Calc
 * spreadsheet of references (alias-aware headers), then renders a formatted
 * ASA reference list with citeproc-js and the official ASA CSL style.
 */
const odsToAsa: Converter = {
  id: "ods-to-asa",
  label: "ODS → ASA",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert ODS to ASA",
        err,
      );
    }
    opts?.onProgress?.(1);
