...
/**
 * XLSX → Chicago. Reads the first sheet of an Excel spreadsheet of
 * references (alias-aware headers, so Scopus/Zotero exports saved as .xlsx
 * work), then renders a formatted Chicago (author-date) reference list with citeproc-js
 * and the official Chicago CSL style. Plain text, one reference per paragraph.
 */
const xlsxToChicago: Converter = {
  id: "xlsx-to-chicago",
  label: "XLSX → Chicago",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert XLSX to Chicago",
        err,
      );
    }
    opts?.onProgress?.(1);
