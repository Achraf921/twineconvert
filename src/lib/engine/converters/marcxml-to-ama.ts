...
/**
 * MARCXML → AMA. Parses a MARCXML export into the unified Citation
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert MARCXML to AMA",
        err,
      );
    }
    opts?.onProgress?.(1);
