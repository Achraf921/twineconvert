...
/**
 * XLSX → Harvard. Reads the first sheet of an Excel spreadsheet of
 * references (alias-aware headers), then renders a formatted Harvard (Cite Them Right) author-date
 * reference list with citeproc-js and the official Harvard CSL style.
 */
const xlsxToHarvard: Converter = {
  id: "xlsx-to-harvard",
  label: "XLSX → Harvard",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert XLSX to Harvard",
        err,
      );
    }
    opts?.onProgress?.(1);
