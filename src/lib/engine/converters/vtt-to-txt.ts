...
/**
 * VTT → Text. Extracts the plain-text transcript (timestamps,
 * indices, and inline markup removed; repeated auto-caption lines
 * de-duplicated).
 */
const vttToTxt: Converter = {
  id: "vtt-to-txt",
  label: "VTT → Text",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert VTT to Text",
        err,
      );
    }
    opts?.onProgress?.(1);
