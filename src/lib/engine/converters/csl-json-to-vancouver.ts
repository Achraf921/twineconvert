...
/**
 * CSL-JSON → Vancouver. Parses CSL-JSON into the unified Citation model, then
 ...
 */
const cslJsonToVancouver: Converter = {
  id: "csl-json-to-vancouver",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert CSL-JSON to Vancouver",
        err,
      );
    }
    opts?.onProgress?.(1);
...
export default cslJsonToVancouver;
