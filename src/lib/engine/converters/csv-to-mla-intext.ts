...
/**
 * CSV → MLA in-text citations. Reads a spreadsheet of references
 * (alias-aware headers), then renders the MLA in-text citation for each
 * row, tab-paired with the source title.
 */
const csvToMlaIntext: Converter = {
  id: "csv-to-mla-intext",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(hint ?? (err instanceof Error ? err.message : "Could not generate MLA in-text citations"), err);
    }
    opts?.onProgress?.(1);
