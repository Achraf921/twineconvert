...
/**
 * XLSX → Vancouver. Reads the first sheet of an Excel spreadsheet of references,
 * then renders a formatted Vancouver (numbered) reference list with citeproc-js
 * and the official Vancouver CSL style.
 */
const xlsxToVancouver: Converter = {
  id: "xlsx-to-vancouver",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert XLSX to Vancouver",
        err,
      );
    }
    opts?.onProgress?.(1);
...
export default xlsxToVancouver;
