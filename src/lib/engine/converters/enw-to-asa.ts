...
/**
 * EndNote (.enw) → ASA. Parses an EndNote export into the unified Citation
 * model, then renders a formatted ASA reference list with citeproc-js and
 * the official ASA CSL style. Lets EndNote users get a formatted ASA
 * bibliography without opening EndNote. Plain text.
 */
const enwToAsa: Converter = {
  id: "enw-to-asa",
  label: "EndNote → ASA",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert EndNote to ASA",
        err,
      );
    }
    opts?.onProgress?.(1);
