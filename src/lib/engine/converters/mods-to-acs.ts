...
/**
 * MODS → ACS. Parses a MODS export into the unified Citation
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert MODS to ACS",
        err,
      );
    }
    opts?.onProgress?.(1);
