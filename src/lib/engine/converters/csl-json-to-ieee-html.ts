...
/**
 * CSL-JSON -> IEEE (HTML). Parses CSL-JSON into the unified Citation model, then
 * renders a formatted IEEE reference list as a self-contained HTML document,
 * ready to paste into Word, Google Docs or a web page with formatting intact.
 * The plain-text IEEE tool is the companion for copy-paste-anywhere output.
 */
const cslJsonToIeeeHtml: Converter = {
  id: "csl-json-to-ieee-html",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      out = await renderCitationStyleHtml(citations, "ieee");
    } catch (err) {
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not render the IEEE HTML bibliography",
        err,
      );
    }
    opts?.onProgress?.(1);
...
export default cslJsonToIeeeHtml;
