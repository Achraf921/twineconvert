...
/**
 * CSV → APA in-text citations. Reads a spreadsheet of references
 * (alias-aware headers), then renders the APA in-text citation for each
 * row, tab-paired with the source title.
 */
const csvToApaIntext: Converter = {
  id: "csv-to-apa-intext",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(hint ?? (err instanceof Error ? err.message : "Could not generate APA in-text citations"), err);
    }
    opts?.onProgress?.(1);
