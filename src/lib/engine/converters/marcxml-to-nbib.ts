...
/**
 * MARCXML → NBIB. Parses MARC21 slim XML (the format library catalogs export)
 * into the unified Citation model, then writes NBIB.
 * Import-only.
 */
const marcxmlToNbib: Converter = {
  id: "marcxml-to-nbib",
  label: "MARCXML → NBIB",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert MARCXML to NBIB",
        err,
      );
    }
    opts?.onProgress?.(1);
