...
/**
 * RefWorks → Harvard. Parses a RefWorks export into the unified Citation
 ...
 */
const refworksToHarvard: Converter = {
  id: "refworks-to-harvard",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert RefWorks to Harvard",
        err,
      );
    }
    opts?.onProgress?.(1);
...
export default refworksToHarvard;
