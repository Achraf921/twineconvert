...
/**
 * BibTeX → Chicago. Parses BibTeX into the unified Citation model, then
 ...
 */
const bibtexToChicago: Converter = {
  id: "bibtex-to-chicago",
  label: "BibTeX → Chicago",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert BibTeX to Chicago",
        err,
      );
    }
    opts?.onProgress?.(1);
...
export default bibtexToChicago;
