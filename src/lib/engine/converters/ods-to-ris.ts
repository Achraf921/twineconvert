...
/**
 * ODS → RIS. Reads the first sheet of the ODS spreadsheet of
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        err instanceof Error ? err.message : "Could not convert ODS to RIS",
        err,
      );
    }
    opts?.onProgress?.(1);
