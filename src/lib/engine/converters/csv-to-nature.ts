...
/**
 * CSV → Nature. Reads a spreadsheet of references (alias-aware headers), then
 * renders a formatted Nature journal style reference list with citeproc-js and the official
 * Nature CSL style.
 */
const csvToNature: Converter = {
  id: "csv-to-nature",
  label: "CSV → Nature",
  ...
  async convert(input, opts) {
    ...
    try {
      ...
      throw new ConvertFailedError(
        hint ?? (err instanceof Error ? err.message : "Could not convert CSV to Nature"),
        err,
      );
    }
    opts?.onProgress?.(1);
